...
sub store ( $self, %options ) {
    ...
    if ( $glVersion >= 4.2 ) {
        ...
        glTexStorage2D( GL_TEXTURE_2D, 1, $options{target_format} || $self->{target_format}, $options{width}, $options{height} );
        glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, $options{width}, $options{height}, $options{source_format} || $self->{source_format}, GL_UNSIGNED_BYTE, $buf );
        ...
        glTexImage2D( GL_TEXTURE_2D, 0, $options{target_format} || $self->{target_format},
            $options{width}, $options{height}, 0, $options{source_format} || $self->{source_format},
